...
class WC_Gateway_Alpha_Masterpass extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_thankyou_alpha', array(
                $this,
                'thankyou_page'
        ));
        // Payment listener/API hook
        add_action('woocommerce_api_wc_gateway_alpha_masterpass', array(
                $this,
                'check_response'
        ));

        // Set the installments array
        $this->installmentsArray = Array(
                100 => 4,
                200 => 8,
                300 => 12
        );
    ...
    {
        // WC_Gateway_Paypal::log( 'Generating payment form for order ' . $order->get_order_number() . '. Notify URL: '
        // . $this->notify_url );
        ...
        $args = array(
                'mid' => $this->MerchantId,
                'lang' => $lang,
                'orderid' => $uniqid . 'AlphaBankOrder' .
                ((WC()->version >= '3.0.0') ? $order->get_id() : $order->id),
                'orderDesc' => 'Name: ' . $order->get_formatted_billing_full_name() . ' Address: ' .
                implode(",", $address),
        ...
        $args = array_merge($args,
                array(
                        'confirmUrl' => add_query_arg('confirm',
                                (WC()->version >= '3.0.0') ? $order->get_id() : $order->id, $return),
                        'cancelUrl' => add_query_arg('cancel',
                                (WC()->version >= '3.0.0') ? $order->get_id() : $order->id, $return)
                ));
    ...
    {
        echo '<p>' .
                __('Thank you - your order is now pending payment. Please click the button below to proceed.',
                        'woocommerce') . '</p>';
        $order = wc_get_order($order_id);
        $uniqid = uniqid();

        $form_data = $this->get_alpha_args($order, $uniqid, 0);
        $digest = base64_encode(sha1(implode("", array_merge($form_data, array(
                'secret' => $this->Secret
        ))), true));
...
<form id="shopform1" name="shopform1" method="POST"
    action="<?php echo $this->AlphaBankUrl ?>" accept-charset="UTF-8">
			<?php

foreach ($html_form_fields as $field)
            echo $field;
        ?>
        ...
        id="submit_twocheckout_payment_form"
        value="<?php echo __( 'Pay via Alpha bank Masterpass', 'woocommerce' ) ?>" />
    <a class="button cancel"
        href="<?php echo esc_url( $order->get_cancel_order_url() )?>"><?php echo __( 'Cancel order &amp; restore cart', 'woocommerce' )?></a>

</form>
<?php

        $order->update_status('pending',
                __('Sent request to Alpha bank with orderID: ' . $form_data['orderid'], 'woocommerce'));
    }
    ...
    public function check_response()
    {
        ...
        if (! isset($_REQUEST['digest'])) {
            wp_die('Alpha Bank Request Failure', 'Alpha Bank Gateway', array(
                    'response' => 500
            ));
        }
        ...
        $string_form_data = array_merge($required_response, array(
                'secret' => $this->Secret
        ));
        $digest = base64_encode(sha1(implode("", $string_form_data), true));

        if ($digest != $_REQUEST['digest']) {
            wp_die('Alpha Bank Digest Error', 'Alpha Bank Gateway', array(
                    'response' => 500
            ));
        }
        ...
        if (isset($_REQUEST['cancel'])) {
            ...
            if (isset($order)) {
                $order->add_order_note(
                        'Alpha Bank Payment <strong>' . $required_response['status'] . '</strong>. txId: ' .
                        $required_response['txId'] . '. ' . $required_response['message']);
            ...
            if (isset($_REQUEST['confirm'])) {
                ...
                if (isset($order)) {
                    if ($required_response['orderAmount'] == wc_format_decimal($order->get_total(), 2, false)) {
                        $order->add_order_note(
                                'Alpha Bank Payment <strong>' . $required_response['status'] . '</strong>. txId: ' .
                                $required_response['txId'] . '. payMethod: ' . $required_response['payMethod'] .
                                '. paymentRef: ' . $required_response['paymentRef'] . '. ' .
                                $required_response['message']);
                        $order->payment_complete(
                                'Alpha Bank Payment ' . $required_response['status'] . '. txId: ' .
                                $required_response['txId']);
                        wp_redirect($this->get_return_url($order));
                        exit();
                    } else {
                        $order->add_order_note(
                                'Payment received with incorrect amount. Alpha Bank Payment <strong>' .
                                $required_response['status'] . '</strong>. ' . $required_response['message']);
                    }
                }
            }

        // something went wrong so die
        wp_die('Unspecified Error', 'Payment Gateway error', array(
                'response' => 500
        ));
    }
        ...
        if ($installments > 0 && is_int($installments)) {
            ...
            for ($i = 0; $i <= $installments; $i ++) {
                ...
                $digest = base64_encode(
                        sha1(implode("", array_merge($form_data, array(
                                'secret' => $this->Secret
                        ))), true));
            ...
            echo '<fieldset class="wc-payment-form">
						<p class="form-row form-row-wide">
							<label for="extInstallmentperiod">' .
                    __('Άτοκες Δόσεις ', 'woocommerce') . ' </label>
							' . $installMentsField . '</p>
						<div class="clear"></div>
					</fieldset>';
